...
class ImportantPhysicsFormulas(LogoScene):
    def construct(self):
        ...
        for idx, item in enumerate(formulas_data, start=1):
            # 提取字段
            name_text = item["name"]
            formula_tex = item["formula"]
            info_text = item["info"]
            formula_color = item.get("color", WHITE)
            scale_factor = item.get("scale", 1.0)

            # 创建对象
            name = Text(name_text, font="Microsoft YaHei", font_size=56, color=YELLOW)
            formula = MathTex(formula_tex, color=formula_color, font_size=54)
